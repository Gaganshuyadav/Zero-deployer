...
async function processBatch({ batch, resolveOffset, heartbeat, commitOffsetsIfNecessary, uncommittedOffsets, isRunning, isStale, pause }) {
  ...
  .map(m => {
    ...
    let parsed: any;
    try {
      parsed = JSON.parse(m.value.toString());
    } catch (err) {
      // if message invalid JSON, mark for DLQ or skip
      return null;
    }
    ...
    return {
      ...
      message: parsed.message ?? m.value.toString(),

      // Metadata
      source: parsed.source ?? 'container',
      container_id: parsed.container_id ?? process.env.HOSTNAME ?? null,
      host: parsed.host ?? os.hostname(),

      // Time
      event_time: parsed.event_time
        ? new Date(parsed.event_time).toISOString()
